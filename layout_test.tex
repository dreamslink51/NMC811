...
\title[NMC]{From Atoms to Cells: Multi-scale Modelling of LiNi\textsubscript{x}Mn\textsubscript{y}Co\textsubscript{z}O\textsubscript{2} (NMC) Cathodes for Li-ion Batteries}
...
%Ni$^{4+}$ (${t_{2g}}^{6}{e_{g}}^{0}$) 
%AW - I am not sure what the floating Ni 4+ above was doing, so I commented it out
...
%    \includegraphics[width=0.75\columnwidth]{Ni_ox_vs_E.pdf}
   \end{tabular}
    ...
    %Reproduced with permissions from \citenum{rana}. 
    }
    \end{center}
\end{figure*}
